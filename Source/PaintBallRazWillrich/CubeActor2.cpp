...
// Sets default values
ACubeActor2::ACubeActor2()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = true;

	// Defining the mesh property:
	cube2 = CreateDefaultSubobject <URuntimeMeshComponentStatic>(TEXT("GeneratedMesh"));
	RootComponent = cube2;
}

// Called when the game starts or when spawned
void ACubeActor2::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void ACubeActor2::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ACubeActor2::PostActorCreated()
{
	Super::PostActorCreated();
	GenerateBoxMesh();
} // This is called when the actor is already in the level and the map is opened.

void ACubeActor2::PostLoad()
{
	Super::PostLoad();
	GenerateBoxMesh();
}

void ACubeActor2::GenerateBoxMesh() {
	...
	cube2->CreateSectionFromComponents(0, 0, 0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, ERuntimeMeshUpdateFrequency::Infrequent, true);
}

void ACubeActor2::CreateBoxMesh(FVector BoxRadius, TArray <FVector>& Vertices, TArray <int32>& Triangles, TArray <FVector>& Normals, TArray <FVector2D>& UVs, TArray <FRuntimeMeshTangent>& Tangents, TArray <FColor>& Colors)
{
	FVector BoxVerts[8];
